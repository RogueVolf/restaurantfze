...
// // TODO: THIS IS FOR EDITING THE TAKEAWAY LOG code from here pls
...
//             <h1>Edit the Order Details for Recipient Id:{id}</h1>
...
//                         <a href="#" className="running-order" style={{ 'color': '#000' }}>OrderNo #{id}</a>
...
//                             <p>Fetch the previous food and add here</p>
...
const TakeAwayEdit = () => {
    ...
    const fetchCurrentOrder = async () => {
        try {
            const response = await axios.get(`https://restogenius.onrender.com/get_bytakeaway_order/${id}`);
    ...
    const SaveTakeAway = async () => {
        const update_customer_details = {
            items_ordered: foodD.length,
            items: foodD,
            total: calculateTotalAmountofItem(),
        }

        console.log(update_customer_details);
        try {
            const updateDetails = await axios.put(`https://restogenius.onrender.com/update_takeaway_order/${id}`,update_customer_details);
    ...
    return (
        <div className='over-order-page'>
            <h1>Get the details of {id}</h1>
            {usercreated ? <button type="button" className='btn btn-primary mt-2 ms-3' onClick={openModal}>
                Add User
            </button> : <div>
                <p className='ms-3 mt-3'>Customer name: <span className='text-capitalize fw-bold'>{fetchFood.customer_name}</span> </p>
                <p className='ms-3'>Customer Phone: <span className='text-capitalize fw-bold'>{fetchFood.customer_mobileNumber}</span></p>
                <p className='ms-3'>Customer email: <span className='text-capitalize fw-bold'>{fetchFood.customer_email}</span></p>
                <p className='ms-3'>Customer Address: <span className='text-capitalize fw-bold'>{fetchFood.customer_address}</span></p>
